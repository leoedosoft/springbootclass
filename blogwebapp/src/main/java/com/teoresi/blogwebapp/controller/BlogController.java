...
public class BlogController {
	...
	public String newBlog(Model model) {
		try {
			...
			throw new RuntimeException(message.getMessage("blogapp.error.controller.newBlog", null, LocaleContextHolder.getLocale())+e);
		}
		return "new_blog";
	}
	...
	public String saveBlog(@Valid @ModelAttribute("blog") Blog blog, BindingResult result) {
		try {
			if(result.hasErrors()) {
				if(blog.getUser().getName().isEmpty()) result.addError(new FieldError("blog","user.name", message.getMessage("blogapp.msg.validation.user.name", null, LocaleContextHolder.getLocale())));
				if(blog.getUser().getSurname().isEmpty()) result.addError(new FieldError("blog","user.surname", message.getMessage("blogapp.msg.validation.user.surname", null, LocaleContextHolder.getLocale())));
				if(blog.getUser().getMail().isEmpty()) result.addError(new FieldError("blog","user.mail", message.getMessage("blogapp.msg.validation.user.mail", null, LocaleContextHolder.getLocale())));
